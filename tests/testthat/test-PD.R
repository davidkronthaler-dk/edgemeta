...
test_that("correct execution for key configurations", {
  conditions <- list(
    list(method = "PCD-full",       level.pi = 0.05, B = 50,   seed = NULL,  method.tau2 = "REML"),
    list(method = "PCD-simplified", level.pi = 0.5,  B = 1000, seed = 5,     method.tau2 = "PM"),
    list(method = "PCD-fixed",      level.pi = 0.99, B = 50,   seed = NULL,  method.tau2 = "REML"),
    list(method = "PCD-full",       level.pi = 0.5,  B = 1000, seed = 5,     method.tau2 = "PM"),
    list(method = "PCD-fixed",      level.pi = 0.01, B = 50,   seed = NULL,  method.tau2 = "REML")
  )
  ...
  r1id <- PredDist(es = es, se = se, method = "PCD-full", seed = 999)
  r2id <- PredDist(es = es, se = se, method = "PCD-full", seed = 999)
...
test_that("handles invalid input types", {
  ...
  expect_error(PredDist(es = 1:5, se = 1:5, method = "PCD-full", method.tau2 = "INVALID"))
  ...
  expect_error(PredDist(es, se, B = 1.5))
  expect_error(PredDist(es, se, B = -Inf))
...
test_that("tolerates small sample sizes", {
  ...
  es <- rnorm(2)
  se <- runif(2, 0.1, 1.5)
  expect_no_error(PredDist(es, se, method = "PCD-full", B = 5))
  expect_no_error(PredDist(es, se, method = "PCD-simplified"))
  expect_error(PredDist(es[1], se[1], method = "PCD-full"))
})
...
test_that("works with different prediction levels and seeds", {
  ...
  for (lvl in levels) {
    for (s in seeds) {
      expect_no_error(
        PredDist(es, se, level.pi = lvl, seed = s, method = "PCD-full")
      )
    }
  }
})
